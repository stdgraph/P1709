...
\begin{itemize}
      ...
      \end{itemize}
      \item \tcode{constexpr} graphs, made with std::array.
...
The initial proposal is for an \tcode{index_adjacency_list}, where vertices are in a random access range
...
\begin{itemize}
      \item Sparse vertex\_id assignment, requiring vertices to be stored in bi-directional containers
            such as \tcode{map} and \tcode{unordered_map}. When doing this, it's more important to use
            vertex references instead of ids to avoid O(log(n)) cost when calling \tcode{edges(g,uid)}. 
