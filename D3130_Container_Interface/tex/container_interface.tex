...
A further benefit of index-based vertex descriptors is that they can refer to a vertex
even when no physical vertex object exists in the container. Graphs that store only edges
(e.g.\ a compressed adjacency list or an implicit grid graph) are identified purely by
vertex index; algorithms and views operate identically whether or not the container
materialises explicit vertex storage.
...
Because descriptors are at most pointer-sized and trivially copyable, CPO signatures
accept them by value. The \tcode{const&} form appears only in \tcode{requires} clauses
of concepts where value copies are not permitted.
{\small
