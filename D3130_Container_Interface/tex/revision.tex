...
\begin{itemize}
      \item \textbf{Mapped vertices and edges}: vertex container may now be associative (e.g.\ \tcode{std::map}, 
            \tcode{std::unordered_map}), enabling adjacency lists with non-integral vertex ids. Likewise,
            an edge container/range may also be associative or include a key-only container such as \tcode{std::set}.
            Related changes:
            \begin{itemize}
