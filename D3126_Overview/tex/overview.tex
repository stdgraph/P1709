...
Parallel graph algorithms are not included in this proposal for a number of reasons.
\begin{itemize}
      \item Parallelism is not beneficial for a particular algorithm, such as for depth-first search.
      \item There is no clear industry standard for a parallel version of an algorithm.
      \item The parallel algorithm is a different algorithm altogether, such as \textit{Delta-Stepping} for shortest paths. 
            Omitting it helps to limit the size of this proposal that is already large.
\end{itemize}
