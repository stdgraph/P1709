...
\phil{Need to add visitor event behavior}
...
There is also the special case where edges form a negative cycle. 
...
\phil{I've duplicated BGL's handling of negative weight cycles by making an additional pass
      though the edges to detect a negative cycle and return a false if found. However, more
      is needed so the caller can determine what vertices make up the cycle. I've added
      debug code for this, but I suspect we will want to expose this in some way.}

\phil{NetworkX has \tcode{negative_edge_cycle} and \tcode{find_negative_cycle}. 
      These are needed if negative weight cycles are a pre-condition, or once a cycle is
      detected?}

\phil{Consider returning \tcode{optional<vertex_id_t<G>>}. If it has an id, then it's the 
      id of the vertex in a negative weight cycle that can be fed into a function that 
      returns the cycle.}

\phil{Need to describe visitor event behavior}

\phil{Bellmain-Ford would benefit from a Sources range overload. Calling it multiple
      times would be very expensive, in comparison to Dijkstra.}
...
\begin{itemdescr}
      ...
      \pnum\returns 
            \begin{itemize}
                  \item \lstinline{true} if shortest paths completed successfully.
                  \item \lstinline{false} when a negative weight cycle has been detected. 
                        The \tcode{on_edge_not_minimized()} event is also called on 
                        the visitor before returning.
            \end{itemize}
      \pnum\throws 
            \begin{itemize}
                  \item \tcode{out_of_range} is thrown when \tcode{source} is not in the range \tcode{0 <= source < num_vertices(graph)}.  \\
