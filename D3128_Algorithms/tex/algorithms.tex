...
\phil{The mandates condition should be part of the requires clause.}
\begin{itemdescr}
      %\pnum\mandates
      \pnum\mandates
            \begin{itemize}
...
\begin{comment}
      ...
      %and their weighted distances to all vertices connected to a single source vertex.
      ...
      constexpr void dijkstra_shortest_paths(
            ...
            Distance&      distance,    // out: distance[uid] of vertex_id uid from source
      ...
      \begin{table}[h!]
            ...
            {\begin{tabular}{l l}
                        ...
                        \hline
                        \textit{Preconditions}  & \tcode{source >= 0                                                                                                               &  & source < size(vertices(g))} \\
                        ...
                        \hline
                        \textit{Postconditions} & \tcode{distance[source] == 0}                                                                                                                                   \\
                                                & \tcode{predecessor[source] == source}.                                                                                                                            \\
                        \hline
                        \textit{\effects}       & \tcode{distance[uid]} will be the shortest, weighted distance of vertex\_id \tcode{uid} from \tcode{source}.                                                    \\
                                                & If \tcode{uid} is not connected to \tcode{source} by any edges then it will have a value of \tcode{dijkstra_infinite_distance()}.                                \\
                                                &                                                                                                                                                               \\
                                                & \tcode{predecessor[uid]} will have the preceding vertex\_id of \tcode{uid} in the weighted shortest path to \tcode{source}                                      \\
      ...
      between vertex \tcode{uid} and vertex \tcode{source}. The distance from \tcode{source} to itself is zero.
