...
\begin{itemdescr}
      ...
      \pnum\throws
            \begin{itemize}
                  \item \lstinline{std::out_of_range} if any source vertex ID is not in
                        \lstinline{[0, num_vertices(g))}.
                  \item \lstinline{std::out_of_range} if \lstinline{size(distances) < num_vertices(g)}.
                  \item \lstinline{std::out_of_range} if \lstinline{size(predecessor) < num_vertices(g)}.
...
single-graph overload for \lstinline{index_bidirectional_adjacency_list} graphs.
...
\begin{itemdescr}
      ...
      \pnum\hardprecond
            \begin{itemize}
                  ...
                  \item
                        \lstinline{size(weight) >= num_vertices(g)}.
                  \item
      ...
      \pnum\preconditions
            \begin{itemize}
                  \item
                        \lstinline{weight_fn}, when provided, must satisfy \lstinline{basic_edge_weight_function<G, WF, range_value_t<Weight>, CompareOp>}.
