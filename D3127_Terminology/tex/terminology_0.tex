...
is available in the wild (from IMDB, for example).  Rather, two types of relationships are available, actor-movie and movie-actor.  See Section~\ref{sec:bipartite} below.
...
Figures~\ref{subfig:airport} and~\ref{subfig:circuit} show two examples of graph models,
a network of airline routes between cities and an electronic circuit.  
The figures indicate the domain-specific data to be modeled and the sets $V$ and $E$ for each graph.
Each figure also includes a
