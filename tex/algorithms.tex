...
\phil{We're using preconditions to define the initial values using \tcode{numeric_limits<T>::max()} or an index.
      I'm seeing less value for having the initialization functions.}
...
% The following table is retained for comparison to the new format.
% After it's been reviewed it can be removed.
\begin{table}[h]
\setcellgapes{3pt}
\makegapedcells
\centering
...
\phil{Requires time/space to evalute predecessors whether they're needed or not. 
      Use \tcode{null_predecessors} special value as a way to disable Predecessors evaluation?}
      ...
      \pnum\preconditions
            \begin{itemize}
                  \item
                        ...
                        \phil{O(1) to evaluate. I vote Yes.}
                  \item
                        ...
                        \phil{"Invalid" means != max()? O(V) to evaluate; too expensive? }
                  \item
...
\phil{What's the \tcode{property} concept?} \\
\phil{I assume \tcode{edge_list_graph} is the edge list we've been discussing.} \\
\phil{\tcode{edgelist_range} is currently being used in the code instead of \tcode{edge_list_graph}. 
I like \tcode{edge_list_graph} or \tcode{edgelist_graph} better.}
