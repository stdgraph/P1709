...
Our proposed set of algorithms are grouped into Tier 1, Tier 2, and Tier 3. All Tier 1 algorithms are included in this proposal and summarized in the lists below. 
Other tiers are outlined in the section \ref{other_algo} Other Algorithms.
...
\begin{comment}
      \phil{Confirm: BFS=pred; Dijkstra=pred+dist, dist}

      \phil{Does the overload with distances overlap the Dijstra implementation? Requires weight function?}

      \phil{Important to have \tcode{q} parameter? Requires Queueable concept. (This was part of earlier discussions.) }

      \phil{Should we have general overloads with Compare and Combine like we do with Dijkstra?}

      \phil{Use shortest\_paths init functions instead of BFS-specific init functions?}
\end{comment}
...
{\small
      \lstinputlisting{src/shortest_paths_helpers.hpp}
}
...
{\small
      \lstinputlisting{src/dijkstra_common.hpp}
      \lstinputlisting{src/dijkstra_general.hpp}
}
...
{\small
      \lstinputlisting{src/bellman_ford_common.hpp}
      \lstinputlisting{src/bellman_ford_general.hpp}
}
...
{\small
      \lstinputlisting[firstline=4,lastline=11]{src/lp.hpp}
}
\begin{itemdescr}
      %\pnum\mandates
      \pnum\preconditions
...
{\small
      \lstinputlisting[firstline=13,lastline=21]{src/lp.hpp}
}
\begin{itemdescr}
      %\pnum\mandates
      \pnum\preconditions
...
{\small
     \lstinputlisting[firstline=23,lastline=25]{src/connected_components.hpp}
}
...
{\small
     \lstinputlisting[firstline=30,lastline=35]{src/connected_components.hpp}
}
\begin{itemdescr}
      %\pnum\mandates
      \pnum\preconditions
...
{\small
     \lstinputlisting[firstline=40,lastline=43]{src/connected_components.hpp}
}
\begin{itemdescr}
      %\pnum\mandates
      \pnum\preconditions
...
{\small
      \lstinputlisting[firstline=4, lastline=9]{src/connected_components.hpp}
}
\begin{itemdescr}
      %\pnum\mandates
      \pnum\preconditions
...
{\small
      \lstinputlisting[firstline=14,lastline=18]{src/connected_components.hpp}
}
\begin{itemdescr}
      %\pnum\mandates
      \pnum\preconditions
...
\begin{table}[h]
      ...
      \hline
            ...
            & \textbf{Throws?} ? & \textbf{Cycles?} No \\
            & \textbf{Multi-edge?} ? & \textbf{Directed?} ?\\
      \hline
      \end{tabular}
      %\caption{Tarjan's SCC Summary}
      \label{tab:topo_sort_algo}
      \end{table}
      
      \phil{Prototype needed}

\phil{Characteristics needed}

\phil{Do we need this as an algorithm, in addition to the view? What value does it bring?}

\phil{Should prototype have a similar interface to BFS with predecessors and/or distance?}
