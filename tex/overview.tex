...
\item Free functions used by Graph Algorithms should be customization point objects, unless there's a good reason not to. Reasonable default implementations should be provided whenever possible.
...
While NWGraph made important strides, 
as a prototype to explore certain implementation aspects,
there are some areas that don't conform to established expectations in the Standard Library. Examples
include using parameter packs to define multiple value types for an edge, rather than a single type; a focus on only defing value types on edges, but not vertices or the graph (NWGraph espouse a philosophy of ``there are no vertices, only vertex ids''); 
because of the focus on graphs as containers of containers,
the API for integrating existing third-party graph containers to be used by the algorithms is currently somewhat limited.
\\
\andrew{These limitations need to be stated a little more carefully.  For some of these, there are simply alternative approaches (such as vertex value types).}
