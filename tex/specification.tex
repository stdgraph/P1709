...
template <typename G>
auto vertex_value(G&& g, vertex_t<G>);
...
template <class G>
concept has_degree = requires(G&& g, vertex_t<G> u) {
  {degree(g, u)};
};
...
template <class G>
concept has_find_vertex_edge = requires(G&& g, vertex_id_t<G> uid, vertex_id_t<G> vid, vertex_t<G> u) {
...
concept adjacency_list = vertex_range<G> && targeted_edge<G, edge_t<G>> &&
      requires(G&& g, vertex_t<G> u, vertex_id_t<G> uid) {
...
template <typename G>
auto incidence(G&& g, vertex_t<G>);

template <typename G>
auto neighbors(G&& g, vertex_t<G>);
